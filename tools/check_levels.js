...
vm.runInContext(fs.readFileSync(path.join(__dirname, '../js/levels.js'), 'utf8') + '\nthis.LEVELS=LEVELS;this.makeBuilder=makeBuilder;', ctx);
...
for (const L of ctx.LEVELS) {
  ...
  const stand = (x, y) => !solid(x, y) && !solid(x, y - 1) && t(x, y) !== '^' && (BLOCK.includes(t(x, y + 1)) || support.has(x + ',' + (y + 1)));
  let S = null, E = null;
  ...
  const reach = (dash) => {
    ...
    while (q.length) {
      ...
      for (const [x2, y2] of nodes) {
        ...
        if (ok && !(spring && dy > 3) && !clear(x, y, x2, y2)) ok = false;
