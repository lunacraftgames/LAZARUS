...
Object.assign(Game, {
  ...
  renderInventory(ctx) {
    ...
    if (d.slot === 'paint' || d.slot === 'trail' || d.slot === 'blade') {
      Inventory.preview = { slot: d.slot, id: d.id };
      ...
      ctx.fillText(d.key === 'dashStrike' ? '⇶' : '⚔', dx + dw / 2, 195); ctx.textAlign = 'left';
    } else {
    ...
    const desc = own ? d.desc : (d.tradable ? '尚未获得。可通过 Boss 掉落、零重构通关、游玩时长掉落获得，或在 Steam 市场与其他玩家交易。' : '通关第一章后获得。');
  ...
  updateSelect() {
    ...
    if (Input.hit('ml') || Input.hit('mr')) { this.selSel = this.selSel < half ? Math.min(n - 1, this.selSel + half) : this.selSel - half; Sound.sfx.select(); }
    if (Input.hit('confirm')) { Sound.sfx.confirm(); this.startLevel(this.selSel); }
    if (Input.hit('pause') || Input.hit('back')) { Sound.sfx.select(); this.toTitle(); }
  },
  renderSelect(ctx) {
    drawBackground(ctx, { x: this.t * 10, y: 0 }, THEMES.hall, this.t);
    ctx.fillStyle = 'rgba(0,0,0,0.65)'; ctx.fillRect(0, 0, VW, VH);
    ctx.fillStyle = '#f2ead6'; ctx.font = 'bold 28px ' + FONT; ctx.fillText('章节选择 · 第一章 死寂博物馆', 70, 90);
    ...
    LEVELS.forEach((L, i) => {
      ...
      const sel = i === this.selSel, x = 70 + col * 420, y = 150 + row * 64, w = 400;
      ...
      this.addHot(x, y, w, 52, () => { Sound.sfx.confirm(); this.startLevel(i); }, () => { this.selSel = i; });
      ...
      const B = makeBuilder(L.w, L.h); L.build(B); const total = B.grid.flat().filter((c) => c === 'o').length;
      const got = [...this.chips].filter((c) => c.startsWith(L.id + '#')).length;
      ctx.textAlign = 'right'; ctx.font = '12px ' + FONT;
      if (!L.boss) { ctx.fillStyle = '#fc6'; ctx.fillText(`芯片 ${got}/${total}`, x + w - 14, y + 24); }
      const tag = L.weapon; if (tag) { ctx.fillStyle = '#f96'; ctx.font = '11px ' + FONT; ctx.fillText(tag, x + w - 14, y + 42); }
      ctx.textAlign = 'left';
    });
    drawHintLine(ctx, 70, VH - 30, [{ k: 'select' }, '选择', { k: 'tab' }, '换列', { k: 'confirm' }, '开始', { k: 'back' }, '返回']);
