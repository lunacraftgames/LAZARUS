...
Object.assign(Game, {
  // 切换到下一把已拥有的武器
  swapWeapon() {
    if (Inventory.charWeapon) { Sound.sfx.denied(); this.toastHint(tr('%{name}只能使用专属的%{w}', { name: tr(this.player.C.name), w: tr(WEAPON_INFO[Inventory.charWeapon].name) })); return; }
