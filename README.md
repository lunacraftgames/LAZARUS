...
- 在培养液里不能吸附。
- **背景音乐随角色调整**（`js/audio.js` 的 `STYLES`）：同一首曲子换一种演奏方式。小扫的「铁皮玩具」风格：速度快约 12%，厚重的锯齿波铺底换成柔和的三角波，旋律高八度、短促跳跃，再加上刷子沙锤、六条腿的三连拍嗒嗒声和小扫的五音动机；阿特拉斯的「钢铁」风格：速度放慢到 0.9 倍，旋律换成滤波锯齿波（像铜管、像机器），低音和底鼓更重，再加上二、四拍的铁砧敲击、液压泄气的嘶声、齿轮的闷响和一段低音铜管动机。标题 / 剧情电台、结局、终局黑暗这类安静的曲目只换音色、保持原速。角色界面里选中哪个角色就会试听它的风格。
- **剧情台词随角色调整**（`js/char_lines.js`）：小扫的周目里，Omni-Mind 把「拉撒路协议」装进了博物馆里一只被踩碎的除尘蜘蛛——开场与结局剧情、伊娃的称呼、能力与武器教学、Omni-Mind 的真相与终局台词、结局铭牌、隐藏结局、第三章 Boss「镜像小扫」都会随之改变；阿特拉斯的周目里，协议被装进了机械展区一台拆掉电池的重型工程外骨骼，冲刺 / 二段跳的教学都改成喷气与猛击，结局铭牌是「展品 #0003 · ATLAS-9」；记忆芯片档案记录的是过去的历史，仍然围绕 LZ-01。新增角色时，在 `CHAR_TEXT` 里写整句改写（`lines`）和称呼替换（`subs`）即可。
- `node tools/check_levels.js` 会分别用小扫（单段跳 + 表面爬行）和阿特拉斯（跳跃 + 喷气悬停）的能力检查每一关能否到达出口。
...
js/lang/en_*.js   英文词典（界面 / 道具 / 第一~四章 / 小扫、阿特拉斯专属台词）
